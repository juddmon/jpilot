 ...
 *
 * Copyright (C) 2001-2026 by Judd Montgomery
 *
...
/* HEADER_NEW_DES_H used to be defined by OpenSSL's des.h itself to flag
 * the pointer-based DES_set_key()/DES_ecb3_encrypt() API. OpenSSL dropped
 * the macro once that became the only API, long before OpenSSL 3, so
 * without this compat define every "#ifdef HEADER_NEW_DES_H" below falls
 * back to lowercase des_*() calls that no longer exist. */
    ...
    if ((gtk_tree_model_get_iter(model, &iter, path)) && (!path_currently_selected)) {
        ...
        if ((record_changed == MODIFY_FLAG) || (record_changed == NEW_FLAG)) {
            ...
            if (b == DIALOG_SAID_3) { /* Save */
                 ...
                 * and crash.  keyring_find() has already reselected the saved
...
int plugin_gui(GtkWidget *vbox, GtkWidget *hbox, unsigned int unique_id) {
    ...
    gtk_grid_attach(GTK_GRID(grid), GTK_WIDGET(entry_password), 1, 3, 1, 1);
    gtk_grid_attach(GTK_GRID(grid), GTK_WIDGET(button), 2, 3, 1, 1);
